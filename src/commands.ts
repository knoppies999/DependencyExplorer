...
import { fetchVersions, isPrerelease } from './services/registryService';
import {
...
async function runFix(
  ...
  const version = await pickVersion(ecosystem, name, currentVersion);
  if (!version) {
    return;
  }
...
async function fixAcrossProjects(tree: DependencyTreeProvider): Promise<void> {
  ...
  await applyAndOffer(ecosystem, name, version, locations);
}
...
async function chooseScope(
  ...
  const originLoc =
    locations.find((l) => l.project === origin) ??
    ({ project: origin, isDirect: false, version: '' } as PackageLocation);

  const items: (vscode.QuickPickItem & { scope: 'all' | 'one' })[] = [
    {
    ...
    },
    {
      ...
      detail: `${originLoc.isDirect ? 'direct' : 'transitive'} ${originLoc.version}`,
      scope: 'one',
    },
  ];
  const picked = await vscode.window.showQuickPick(items, {
  ...
  return picked.scope === 'all' ? locations : [originLoc];
}
...
async function applyAndOffer(
  ...
  await offerInstall(applied, ecosystem, `${name} set to ${version} in ${where}.`);
}
...
async function offerInstall(
  ...
  const installCmd = ecosystem === 'npm' ? 'npm install' : 'dotnet restore';
  const dirs = [...new Set(projects.map((p) => path.dirname(p.manifestPath)))];
  const scope = dirs.length === 1 ? '' : ` in each of the ${dirs.length} projects`;
  const action = await vscode.window.showInformationMessage(
    `${message} Run "${installCmd}"${scope} to apply it — the tree refreshes automatically afterwards.`,
    `Run ${installCmd}`
  );
  if (!action) {
    return;
  ...
  for (const dir of dirs) {
    const terminal = vscode.window.createTerminal({
      name: `${installCmd} · ${path.basename(dir)}`,
      cwd: dir,
    });
    terminal.show();
